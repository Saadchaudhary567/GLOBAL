...
function renderGrid(){
  ...
  $("allProductsGrid").innerHTML=visible.map(p=>{
    ...
    return `<article class="product-card size-${esc(p.cardSize||size)} imgsize-${esc(p.imageSize||size)}"><div class="product-image fit-${esc(p.imageFit||"contain")}" style="${imageAreaStyle(p)}">${productBadges(p)}${productImage(p)}</div><div class="product-body"><div class="rating">${stars||"☆"} ${esc(p.rating||"")}${p.reviewCount?` <span>(${esc(p.reviewCount)})</span>`:""}</div><h3>${esc(p.name)}</h3>${p.price?`<div class="price">${esc(p.price)} ${p.previousPrice?`<del>${esc(p.previousPrice)}</del>`:""}</div>`:""}<p>${esc(p.description||p.text||"")}</p><div class="pc-actions"><button class="button secondary" data-view-product="${p.id}">View Review</button>${p.affiliateUrl||p.url?`<a class="button primary" href="${esc(p.affiliateUrl||p.url)}" target="_blank" rel="nofollow sponsored noopener">${esc(p.buttonText||"Check Price")}</a>`:""}</div></div></article>`;
  }).join("");
...
function openProduct(id){
  ...
  if(!p)return;
  $("modalContent").innerHTML=`<p class="eyebrow">KITCHENZEN PRODUCT REVIEW</p><h2>${esc(p.name)}</h2>${productHtml(p)}`;
  showModal();
